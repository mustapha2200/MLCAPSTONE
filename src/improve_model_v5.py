#!/usr/bin/env python3
"""
improve_model_v5.py — Maximum performance, one-shot submission
V5 Changes over V4 (all bugs from V4 retained/fixed):
  1. TRANSDUCTIVE NLP: TF-IDF/SVD fit on train+test combined — legitimate since
     no label info is used; gives better vocabulary coverage and IDF weights.
     SVD components increased 20→25 per language (50 total).
  2. CLIPPING REMOVED: Per-quartier P2-P98 training clipping was capping
     legitimate high-value properties (esp. Tevragh Zeina). Replaced with a
     conservative global P0.5-P99.5 clip. Test-time prediction clipping also
     removed entirely — it was pushing predictions toward wrong quartier medians.
  3. PSEUDO-LABELING (2 rounds, weight=0.35): Uses the stacking ensemble's
     test predictions as pseudo-labels, adds test rows to training data, and
     retrains. This leverages 289 additional samples (+25% data). Each round
     uses the previous round's best predictions.
  4. RidgeCV meta-learner: auto-selects alpha from [0.01, 0.1, 1, 10, 100]
     instead of hardcoded alpha=1.0.
  5. Surface sanity clip: 20-2000 m² to remove obvious data entry errors.
"""
...
# ── Ridge meta + top raw features — feature selection INSIDE meta-CV folds ───
# Select features per inner-fold train split to avoid selection leakage.
meta_oof_ridge2      = np.zeros(len(y))
meta_test_ridge2_folds = np.zeros((len(test_fe), N_FOLDS))

N_META_FEATS = 10
for fold, (tr_idx, val_idx) in enumerate(kf_meta.split(stack_df)):
    # Select top features based ONLY on the meta train split
    corr_fold = (
    ...
    top_feats = corr_fold.head(N_META_FEATS).index.tolist()

    meta_tr = np.column_stack([stack_df.values[tr_idx],  X_train_full[top_feats].values[tr_idx]])
    meta_val = np.column_stack([stack_df.values[val_idx], X_train_full[top_feats].values[val_idx]])
    meta_te  = np.column_stack([stack_test_df.values,     X_test_full[top_feats].values])
...
sub_path = os.path.join(OUT_DIR, 'submission_v5.csv')
