#!/usr/bin/env python3
"""
improve_model_v7b.py — V6b + early stopping fix only
One targeted change over V6b:
  - EARLY STOPPING IN OOF STACKING LOOP: LGB/XGB/CatBoost were running all
    n_estimators in the stacking OOF loop with no early stopping. From V6b
    output: LGB Optuna best=0.5775 but OOF=0.6032 (gap of 0.026 from this
    alone). XGB: 0.5743 vs 0.5831. Fix: pass eval_set + early_stopping to
    each booster in the OOF loop, matching what Optuna did.
  Everything else is identical to V6b (0.56243 LB).
"""
...
for name, model_fn in base_model_factories.items():
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(train_fe)):
        ...
        # Early stopping for boosting models — matches what Optuna used
        if name == 'lgb':
            model.fit(X_tr, y_tr,
...
sub_path = os.path.join(OUT_DIR, 'submission_v7b.csv')
