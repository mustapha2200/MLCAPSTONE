#!/usr/bin/env python3
"""
improve_model_v4.py — Bugfix release
V4 Fixes over V3:
  BUG 1 (CRITICAL): Meta-learner was evaluated in-sample (fit on OOF, predict
                    on same OOF → artificially reported 0.55 CV). Now all meta-
                    learners use a proper inner kf_meta CV loop.
  BUG 2 (SIGNIFICANT): Optuna trials and final CV evaluation shared the same
                    KFold splits (kf), causing hyperparameter overfit to those
                    exact folds. Now Optuna uses kf_tune (different seed).
  BUG 3 (MINOR): Raw feature selection for meta-learner used full-y correlation
                    (leakage). Now recomputed inside each meta-fold.
"""
